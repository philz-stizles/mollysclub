...
public class CSVHelper {
  ...
  public static List<Patient> csvToPatients(InputStream is) {
    try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(is, "UTF-8"));
      ...
      List<Patient> tutorials = new ArrayList<>();

      Iterable<CSVRecord> csvRecords = csvParser.getRecords();

      for (CSVRecord csvRecord : csvRecords) {
        Patient tutorial = new Patient(
  ...
  public static ByteArrayInputStream tutorialsToCSV(List<Patient> tutorials) {
    final CSVFormat format = CSVFormat.DEFAULT.withQuoteMode(QuoteMode.MINIMAL);

    try (ByteArrayOutputStream out = new ByteArrayOutputStream();
        CSVPrinter csvPrinter = new CSVPrinter(new PrintWriter(out), format);) {
      for (Patient tutorial : tutorials) {
