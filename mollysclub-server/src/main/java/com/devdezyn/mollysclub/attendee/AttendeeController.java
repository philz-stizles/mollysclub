...
@Api(tags = "Attendee")
@RestController
@RequestMapping(path = "/api/v1/attendees")
@RequiredArgsConstructor
@Validated // class level
public class AttendeeController {
  private AttendeeService appointmentService;
  ...
  public ResponseEntity<ApiResponseBody<?>> createAppointment(@RequestBody @Valid AttendeeDto appointmentDto) {

    // Create appointment
    AttendeeDto dto = appointmentService.create(appointmentDto);
  ...
  @GetMapping(path="{id}")
  public ResponseEntity<AttendeeDto> get(@PathVariable @Min(1) Long id) {
    AttendeeDto appointmentDto = appointmentService.findById(id);
  ...
  public ResponseEntity<List<AttendeeDto>> getAll() {

    return new ResponseEntity<List<AttendeeDto>>(appointmentService.findAll(), HttpStatus.OK);
  }
